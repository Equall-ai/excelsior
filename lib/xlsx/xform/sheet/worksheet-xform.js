...
// Helper to strip namespace prefix (e.g., 'x:worksheet' -> 'worksheet')
...
class WorkSheetXform extends BaseXform {
  ...
  parseOpen(node) {
    ...
    if (this.map[cleanName] && !this.ignoreNodes.includes(cleanName)) {
      this.parser = this.map[cleanName];
      this.parser.parseOpen(node);
    }
    return true;
  }
  ...
  reconcile(model, options) {
    ...
    const rels = (model.relationships || []).reduce((h, rel) => {
      ...
      if (rel.Type === RelType.VmlDrawing && model.comments && model.comments.length) {
        const vmlComment = options.vmlDrawings[rel.Target] && options.vmlDrawings[rel.Target].comments;
